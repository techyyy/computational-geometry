...
def read_edges(file_name):
    ...
    i = 0
    while i < len(input_array):
        edges.append(Edge(vertices[int(input_array[i])], vertices[int(input_array[i + 1])]))
        i += 2
    return edges
        ...
        for e in chains[p]:
            if e.start.y < point.y < e.end.y:
                ...
                if math.atan2(point_vector.y, point_vector.x) > math.atan2(edge_vector.y, edge_vector.x):
                    return "The point is between chains " + str(p - 1) + " , " + str(p)
    return "The point is not inside the graph"


vertices = read_points("C:\\Users\\lunae\\PycharmProjects\\comp_graphics\\lab_1\\points.txt")
edges = read_edges("C:\\Users\\lunae\\PycharmProjects\\comp_graphics\\lab_1\\edges.txt")
